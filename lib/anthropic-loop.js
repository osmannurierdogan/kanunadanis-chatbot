    ...
    for await (const event of stream) {
      ...
      if (event.type === 'content_block_start') {
        ...
        if (cb.type === 'text') {
          ...
          // emit a placeholder so frontend reserves the timeline slot in order
          onEvent({ type: 'tool_pending', name: cb.name, id: cb.id });
        } else {
        ...
        if (block && block.type === 'tool_use') {
          ...
          // input artık parse edildi — args ile birlikte tool_start emit et
          onEvent({ type: 'tool_start', name: block.name, id: block.id, input: block.input });
        }
    ...
    for (const tu of toolUses) {
      try {
        ...
        const textContent = Array.isArray(result.content)
          ...
          : '';
        const resultPreview = textContent || JSON.stringify(result);
        onEvent({
          type: 'tool_end',
          name: tu.name,
          id: tu.id,
